...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(BASIC_SEX))==(code_SEX: SEX >->> 0..1 & decode_SEX: 0..1 >->> SEX & decode_SEX = code_SEX~ & SEX: FIN(INTEGER) & not(SEX = {}))
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(BASIC_SEX)) == (Type(man) == Cst(etype(SEX,0,1));Type(woman) == Cst(etype(SEX,0,1));Type(code_SEX) == Cst(SetOf(etype(SEX,0,1)*btype(INTEGER,0,1)));Type(decode_SEX) == Cst(SetOf(btype(INTEGER,0,1)*etype(SEX,0,1))))
END
&
