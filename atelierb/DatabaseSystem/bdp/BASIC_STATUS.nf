...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(BASIC_STATUS))==(code_STATUS: STATUS >->> 0..1 & decode_STATUS: 0..1 >->> STATUS & decode_STATUS = code_STATUS~ & STATUS: FIN(INTEGER) & not(STATUS = {}))
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(BASIC_STATUS)) == (Type(dead) == Cst(etype(STATUS,0,1));Type(living) == Cst(etype(STATUS,0,1));Type(code_STATUS) == Cst(SetOf(etype(STATUS,0,1)*btype(INTEGER,0,1)));Type(decode_STATUS) == Cst(SetOf(btype(INTEGER,0,1)*etype(STATUS,0,1))))
END
&
