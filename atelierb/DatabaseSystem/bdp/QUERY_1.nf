...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Implementation(QUERY_1),get_new_dead_person)==(btrue | ("Person Id:": STRING | skip);(btrue | @(pp$0).(pp$0: PERSON ==> pp:=pp$0));(pp: PERSON | bb:=bool(pp: person));(bb = FALSE ==> ("Person does not exist\n": STRING | skip) [] not(bb = FALSE) ==> ((pp: PERSON & pp: person | bb:=bool(pp: status~[{living}]));(bb = FALSE ==> ("Person must be alive\n": STRING | skip) [] not(bb = FALSE) ==> skip))));
  List_Substitution(Implementation(QUERY_1),get_new_dead_person)==(STRING_WRITE("Person Id:");pp <-- PERSON_read;bb <-- is_present(pp);IF bb = FALSE THEN STRING_WRITE("Person does not exist\n") ELSE bb <-- is_living(pp);IF bb = FALSE THEN STRING_WRITE("Person must be alive\n") END END);
